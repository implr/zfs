...
main(int argc, char **argv)
{
	...
	/*
	 * open_by_handle_at() needs a "mount fd": any fd that resides on the
	 * same filesystem as the handle, used to identify the target mount.
	 * Open the containing directory and use it, otherwise the handle would
	 * be resolved against the wrong filesystem (e.g. the caller's cwd).
	 */
	...
	int dirfd = open(dirname(pathdup), O_RDONLY | O_DIRECTORY);
	free(pathdup);
	if (dirfd < 0) {
		fprintf(stderr, "open parent dir of %s: %s\n",
		    path, strerror(errno));
		return (1);
	}
	...
	if (dc >= 0) {
		/* best-effort cache drop; failure is not fatal to the test */
		if (write(dc, "2\n", 2) < 0)
			(void) fprintf(stderr, "drop_caches write failed: %s\n",
			    strerror(errno));
		close(dc);
	}

	/* The actual assertion: open by handle must succeed. */
	int fd2 = open_by_handle_at(dirfd, &h.fh, O_RDONLY);
	if (fd2 < 0) {
		fprintf(stderr,
